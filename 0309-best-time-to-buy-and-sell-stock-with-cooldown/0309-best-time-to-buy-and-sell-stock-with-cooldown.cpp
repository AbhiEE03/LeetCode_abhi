...
public:
    int maxProfit(vector<int>& prices) {
        ...
        for (int i = n - 1; i >= 0; i--) {
            ...
            // selling curr + bought next, didn't sell curr + selling the next
...
//             curr_state[i][1] = max(-prices[i] + next_state[0], next_state[1]);

//             // profit = sell curr + buy next, sell next
//             curr_state[i][0] = max(prices[i] + next_state[0], next_state[0]);
//         }
